...
async function loadDashboard() {
  try {
    ...
    // 🧑‍🏫 Trainer Details — DYNAMIC from DB
    const trainerName = data.assigned_trainer || "Assigned soon";
    const trainerSlot =
      trainerName.toLowerCase() === "no trainer"
        ? "N/A"
        : trainerName.toLowerCase() === "assigned soon"
        ? "TBD"
        : "Morning / Evening (As per schedule)";

    document.querySelector(".trainer-name").textContent = trainerName;
    document.querySelector(".trainer-slot").textContent = trainerSlot;
